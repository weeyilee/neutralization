...
def get_examples(data_path, tok2id, max_seq_len, 
    ...
    for i, (line) in enumerate(tqdm(open(data_path, encoding="utf8"))): ### added encoding="utf8"
        ...
        # broken line
            ...
            skipped += 1
            skipped_list.append(['parts_count', f'i: {i}', f'revid: na']) ### MODIFIED
        ...
        if len(tokens) != len(pre_tok_labels) \
            ...
            skipped += 1
            skipped_list.append(['len_error', f'i: {i}', f'revid: {revid}']) ### MODIFIED
            continue
        ...
        if len(tokens) > max_seq_len - 1 or len(post_tokens) > max_seq_len - 1:
            skipped += 1
            skipped_list.append(['post_error', f'i: {i}', f'revid: {revid}']) ### MODIFIED
            continue
        ...
        try:
            ...
            skipped += 1
            skipped_list.append(['encoding_error', f'i: {i}', f'revid: {revid}']) ### MODIFIED
            continue
        ...
        out['revid'].append(i) ### MODIFIED
        

    print('SKIPPED ', skipped)
    print(skipped_list) ### MODIFIED
    print('END skipped_list') ### MODIFIED
    return out
...
def get_dataloader(data_path, tok2id, batch_size, 
    ...
    data = TensorDataset(
        ...
        torch.tensor(examples['revid'], dtype=torch.long))         ### MODIFIED
