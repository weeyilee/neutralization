...
class BilinearAttention(nn.Module):
    ...
    def forward(self, query, keys, mask=None, values=None):
        ...
        if mask is not None:
            attn_scores = attn_scores.masked_fill(mask.bool(), -float('inf')) # ADDED .bool()
...
class LSTMEncoder(nn.Module):
    ...
    def forward(self, src_embedding, srclens, srcmask):
        ...
        if self.pack:
            inputs = pack_padded_sequence(src_embedding, srclens.cpu(), batch_first=True) # MODIFIED ADDED CPU()
        else:
