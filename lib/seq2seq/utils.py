...
def dump_outputs(src_ids, gold_ids, predicted_ids, gold_tok_dist, id2tok, out_file, revid,
        pred_dists=None):
    out_hits = []
    preds_for_bleu = []
    golds_for_bleu = []
    ...
    for src_seq, gold_seq, pred_seq, gold_dist, pred_dist, single_revid in zip(
        src_ids, gold_ids, predicted_ids, gold_tok_dist, pred_dists, revid):
        ...
        print(single_revid, src_seq, pred_seq, list(pred_replace), ### MODIFIED
              sep='\t', end='\n', file=out_file) 
        # print('IN SEQ: \t', src_seq.encode('utf-8'), file=out_file)
        # print('PRED SEQ:\t', pred_seq.encode('utf-8'), file=out_file)
        # print('PRED TOK: \t', list(pred_replace), file=out_file)
